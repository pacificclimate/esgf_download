'''
The esgf_download module includes classes for both retrieving metadata and downloading data. It makes use of the ESGF JSON query interface to determine which servers and data sets to query, then queries the XML for each data set from each of the THREDDS servers.
'''
...
def get_request(requests_object, url, **kwargs):
    ...
    This function performs an HTTP GET request with a supplied session object, returning application-appropriate exceptions where applicable. This allows for download of data with authentication.
...
class MultiFileWriter:
    '''
    A write serializer which allows for many files to be open but for only one to be written to at once. The goal of this is to keep filesystem thrash to a minimum while downloading.
    '''
    ...
    def process(self):
        '''
        Should only be called by the constructor at creation time. Performs the dequeueing and writing.
        '''
    ...
    def enqueue(self, fd, res, last=False):
        ...
        :param last: A flag to specify that this is the last block, and the file descriptor should be closed after it is written out.
        '''
    ...
    def write_and_quit(self):
        '''
        Writes out remaining blocks in the queue and informs the writer thread that it should quit.
        '''
        ...
        # Add a dummy entry after setting run_writer_thread to False so the thread gets awakened
        # and then can die peacefully.
        self.pool_full_sema.acquire();
        with self.lock:
...
class DownloadThread:
    '''
    A downloader which, upon creation, starts up a thread which downloads the specified data, allowing the spawning process to continue its business. Checks for errors and reports them in the event_queue.
    '''
    def __init__(self, url, host, transfert_id, filename, checksum, checksum_type, writer, event_queue, session):
        '''
        ...
        :param event_queue: A Queue to put events (failures to download, successes, corruption) in.
    ...
    def download(self):
        ...
        try:
            ...
            for chunk in res.iter_content(self.blocksize):
                ...
                this_time = time.time()
                self.event_queue.put(("SPEED", self.transfert_id, len(chunk) / (1024.0 * (this_time - last_time))))
        ...
        self.event_queue.put(("DONE", self.transfert_id, (self.data_size / 1024) / (self.start_time - self.end_time)))
...
class Downloader:
    '''
    A downloader which downloads files as specified in the database file, with the authentication credentials provided.
    '''
    def __init__(self, database_file, base_path, username, password, auth_server, initial_threads_per_host=3, max_total_threads=100, **kwargs):
        '''
        Creates a Downloader object.
        :param database_file: Sqlite3 database file where information is stored on files to be downloaded.
    ...
    def metadata_reader(self):
        '''
        Routine which retrieves metadata and places it into a queue to be processed. Spawned as a thread. Internal.
        '''
        ...
        # FIXME: The method used here is kind of wrong. It should check for anything that's changed; but instead it only checks for stuff that's new.
        while self.running:
            try:
                with self.database_lock:
                    for row in curse.execute("SELECT transfert.*,model.* from transfert JOIN model ON model.name=transfert.model WHERE status = 'waiting' AND transfert_id > ?", [last_transfert_id]):
    ...
    def handle_events(self):
        ...
        while not self.event_queue.empty():
            ...
            if ev == "ERROR":
                ...
                # Specifically, something that behaves differently depending on the error message so that we can realize when a connection's been reset, etc, and can respond appropriately by scaling back # threads.
    ...
    def go_get_em(self):
        '''
        Routine which spawns the MultiFileWriter, performs authentication, spawns the MetadataReader, and begins downloading data.
        '''
        # Set stuff to run.
        self.running = True
        self.stop_now = False
        ...
        while self.running:
            try:
                ...
                for hostname, host in self.hosts.items():
                    while (len(host.download_queue) != 0) and host.thread_count < host.max_thread_count and self.total_threads < self.max_total_threads:
                        item = host.download_queue.popleft()
                        self.download_threads[item['transfert_id']] = DownloadThread(item['location'], item['datanode'], item['transfert_id'], self.base_path + "/" + item['local_image'], item['checksum'], item['checksum_type'], writer, self.event_queue, host.session)
                        host.thread_count += 1
                        self.total_threads += 1
                        self.handle_events()
                        time.sleep(0.2)
        ...
        if self.stop_now:
            ...
            for dt in self.download_threads.values():
                with dt.abort_lock:
                    dt.abort = True
                self.conn.execute("UPDATE transfert SET status='waiting' where transfert_id = ?", [dt.transfert_id])
            self.conn.commit()
...
def get_property_dict(xml_tree, xpath_text='ud:property', namespaces={'ud':'http://www.unidata.ucar.edu/namespaces/thredds/InvCatalog/v1.0'}):
    '''
    Performs the given xpath query on the given xml_tree, with the given namespace, returning a dictionary.
...
# Try using select()?
def metadata_update(database_file, search_host="http://pcmdi11.llnl.gov/esg-search/search", **constraints):
    '''
    Queries the ESGF server for a set of datasets, queries each THREDDS server for metadata for each data set (the list of files), and records information about datasets and data files in the given database file.
    ...
    field_map_model = { 'data_node': 'datanode', 'institute': 'institute', 'model': 'name' }
    field_map_transfert = { 'model': 'model', 'checksum': 'checksum', 'size': 'fsize', 'variable': 'variable', 'tracking_id': 'tracking_id', 'version': 'version_xml_tag', 'size': 'size_xml_tag', 'checksum_type': 'checksum_type', 'product': 'product_xml_tag', 'product': 'local_product', 'local_image': 'local_image', 'status': 'status', 'location': 'location' }

    model_fetch_query = "SELECT name from model where name = ?"
    model_insert_query = "INSERT INTO model(" + ",".join(field_map_model.values()) + ") VALUES(" + ",".join(["?"] * len(field_map_model)) + ")"
    transfert_fetch_query = "SELECT transfert_id from transfert where tracking_id = ?"
    transfert_insert_query = "INSERT INTO transfert(" + ",".join(field_map_transfert.values()) + ") VALUES(" + ",".join(["?"] * len(field_map_transfert)) + ")"

    output_path_json_bits = [ 'project', 'product', 'institute', 'clean_model', 'experiment', 'time_frequency', 'realm', 'cmor_table', 'ensemble', 'version', 'variable', 'filename' ]
    ...
    get_thredds_server_base = etree.XPath("/ud:catalog/ud:service[@name='fileservice' or @name='fileService']/ud:service[@name='HTTPServer' or @serviceType='HTTPServer']", namespaces=ns)
    get_thredds_server_base_alt = etree.XPath("ud:service[@name='HTTPServer' or @serviceType='HTTPServer']", namespaces=ns)
    ...
    for ds0 in ds:
        ...
        filter_elements = etree.XPath("/ud:catalog/ud:dataset/ud:dataset[ud:serviceName='" + thredds_httpserver_service_name + "']/ud:variables/ud:variable[" + " or ".join(["@name='%s'" % var for var in constraints['variable'] ]) + "]/../..", namespaces=ns)
        ...
        for ds_file in matches:
            ...
            if(num_results == 0):
                ...
                if len(missing_keys) > 0:
                    log.warning("Error: dataset object " + metadata['location'] + " will be omitted as it is missing the following keys: " + ",".join(missing_keys))
                    continue
